...
| FFmpeg, ffprobe, yt-dlp, Deno, and yt-dlp-ejs are packaged | Verified | The packaged API reports all tools; packaging now fails when FFmpeg/ffprobe are absent; final hashes are recorded in `MIGRATION_VERIFICATION_2.0.1.md`. |
| YouTube 403 regression is resolved | Verified | The user-supplied URL `https://www.youtube.com/watch?v=k7nje82oYhI` completed through source and packaged sidecars, including a 4K packaged MP4 validated by bundled ffprobe. |
